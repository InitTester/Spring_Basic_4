...
public class CommentController {
    ...
    public ResponseEntity<String> modify(@PathVariable Integer cno, @RequestBody CommentDto dto) {
        ...
        try {
            if(service.modify(dto)!=1)
                throw new Exception("Modity Failed.");

            return new ResponseEntity<>("MOD_OK",HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>("MOD_ERR",HttpStatus.BAD_REQUEST); // 400
        }
    }
    ...
    public ResponseEntity<String> write(@RequestBody CommentDto dto, Integer bno, HttpSession session) {
        ...
        try {
            ...
            e.printStackTrace();
            return new ResponseEntity<>("WRT_ERR",HttpStatus.BAD_REQUEST); // 400
        }
    }

        // 지정된 댓글을 삭제하는 메서드
    @DeleteMapping("/comments/{cno}") // /comments/1?bno=2112 <-- 삭제할 댓글 번호
